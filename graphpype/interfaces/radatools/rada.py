...
class NetPropRada(CommandLine):
    ...
    def _list_outputs(self):
        ...
        if "all" in self.inputs.optim_seq:
            outputs["global_file"] = os.path.join(
                path, fname + '-info_global.txt')
            outputs["dists_file"] = os.path.join(
                path, fname + '-info_dists.txt')
            outputs["degrees_file"] = os.path.join(
                path, fname + '-info_degrees.txt')
            outputs["nodes_file"] = os.path.join(
                path, fname + '-info_nodes.txt')
            outputs["edges_betw_file"] = os.path.join(
                path, fname + '-info_edges_betw.net')
